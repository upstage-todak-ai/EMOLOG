...
2. **구체적 관찰과 패턴을 포함**하세요 (예: "시험 전에는 불안했지만 시험 후에는 평온했다")
3. **날짜를 명시적으로 언급하지 마세요** (예: "12월 15일", "며칠 며칠" 같은 표현 금지)
   - 대신 "전날", "그 다음날", "시험 전후" 같은 상대적 표현 사용
   - 사용자는 "근거 보기" 버튼으로 해당 날짜의 메모를 확인할 수 있습니다
4. **개인적이고 따뜻한 톤**을 유지하세요 ("~했어요", "~드네요" 등)
5. **3-5 문단** 정도의 길이로 작성하세요
6. **인사이트를 그대로 나열하지 말고**, 자연스러운 이야기 흐름으로 연결하세요
...
=== 출력 JSON 형식 ===
{{
  "report": "인사이트를 바탕으로 작성한 자연스러운 리포트 (3-5 문단, 날짜 언급 없이 구체적 관찰 포함)",
  "summary": "가장 중요한 패턴 한 문장 요약"
}}

**중요**: 리포트 본문에는 절대 날짜(예: "12월 15일", "며칠 며칠")를 포함하지 마세요. 사용자가 보기 어렵고 혼란스러울 수 있습니다. 대신 패턴과 감정 변화를 중심으로 작성하세요."""
